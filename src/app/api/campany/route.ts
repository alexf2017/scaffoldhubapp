...
import { campanyCreateController } from 'src/features/campany/controllers/campanyCreateController';
import { campanyDestroyManyController } from 'src/features/campany/controllers/campanyDestroyManyController';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const payload = await campanyCreateController(body, context);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const payload = await campanyFindManyController(query, context);
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
  }
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    context = await appContext(request);
    const query = queryToObject(request.nextUrl.search);
    await campanyDestroyManyController(query, context);
    const payload = true;
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
