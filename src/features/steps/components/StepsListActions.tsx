...
import {
  stepsDestroyManyApiCall,
  stepsFindManyApiCall,
} from 'src/features/steps/stepsApiCalls';
import { stepsExporterMapper } from 'src/features/steps/stepsExporterMapper';
import { stepsFilterInputSchema } from 'src/features/steps/stepsSchemas';
...
export default function StepsListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof stepsFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.stepsCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.stepsImport, context);

  const exportMutation = useMutation({
    mutationFn: () => {
      return stepsFindManyApiCall({
        filter: filter,
    ...
    onSuccess: (data) => {
      csvExporter(
        stepsExporterMapper(data.steps, context),
        dictionary.steps.fields,
        'steps',
      );
      toast({
        description: dictionary.steps.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.steps.destroyMany.noSelection);
      }

      return stepsDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.steps.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/steps/importer`} prefetch={false}>
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.steps.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.steps.destroyMany.confirmDescription,
            selectedCount,
          )}
