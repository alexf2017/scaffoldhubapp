...
import StepsListActions from 'src/features/steps/components/StepsListActions';
import StepsListFilter from 'src/features/steps/components/StepsListFilter';
import { stepsFindManyApiCall } from 'src/features/steps/stepsApiCalls';
import {
  StepsWithRelationships,
  stepsFilterInputSchema,
...
import { StepsNewButton } from 'src/features/steps/components/StepsNewButton';
import { z } from 'zod';
import { Steps } from '@prisma/client';

const defaultData: Array<any> = [];

export default function StepsList({ context }: { context: AppContext }) {
  ...
  const filter = useMemo(() => {
    return DataTableQueryParams.getFilter<
      z.input<typeof stepsFilterInputSchema>
    >(searchParams, stepsFilterInputSchema);
  }, [searchParams]);

  const columns: ColumnDef<StepsWithRelationships>[] = [
    {
    ...
    },
    {
      accessorKey: 'title',
      meta: {
        title: dictionary.steps.fields.title,
      },
      cell: ({ getValue, row }) => (
  ...
  const query = useQuery({
    queryKey: ['steps', 'list', filter, sorting, pagination],
  ...
  const table = useReactTable({
    ...
    data: query.data?.steps || defaultData,
    columns,
  ...
  return (
    <div className="mb-4 flex w-full flex-col gap-4">
      <div className="flex items-center justify-between">
        <Breadcrumb items={[[dictionary.steps.list.menu]]} />
      ...
      <StepsListFilter context={context} isLoading={query.isLoading} />
      ...
      <DataTable
        ...
        notFoundText={dictionary.steps.list.noResults}
        newButton={<StepsNewButton context={context} />}
      />
