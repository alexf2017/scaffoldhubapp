...
import { stepsFilterFormSchema } from 'src/features/steps/stepsSchemas';
...
function StepsListFilter({
  ...
  const previewRenders = {
    title: {
      label: dictionary.steps.fields.title,
    },
  };

  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof stepsFilterFormSchema>>(
        searchParams,
        stepsFilterFormSchema,
      ),
    [searchParams],
  );

  const form = useForm({
    resolver: zodResolver(stepsFilterFormSchema),
    mode: 'onSubmit',
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.steps.fields.title}</FormLabel>
...
export default StepsListFilter;
