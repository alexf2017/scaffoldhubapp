...
async function goToStepsNew(page: Page) {
  ...
  await page.goto(`/steps/new`);
  await page.waitForURL(`/steps/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToStepsNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.steps.new.title }),
  ).toBeTruthy();
});
