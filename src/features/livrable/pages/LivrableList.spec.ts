...
async function goToLivrableList(page: Page) {
  ...
  // TODO: Create livrable here
  // await prisma.livrable.create({});

  await page.waitForURL('/');
  await page.goto(`/livrable`);
  await page.waitForURL(`/livrable`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToLivrableList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.livrable.list.title }),
  ).toBeTruthy();
});
