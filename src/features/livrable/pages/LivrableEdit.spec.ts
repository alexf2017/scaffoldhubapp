...
async function goToLivrableForm(page: Page) {
  ...
  await page.goto(`/livrable/${livrable.id}/edit`);
  await page.waitForURL(`/livrable/${livrable.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToLivrableForm(page);
});
