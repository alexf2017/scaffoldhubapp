...
async function goToLivrableNew(page: Page) {
  ...
  await page.goto(`/livrable/new`);
  await page.waitForURL(`/livrable/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToLivrableNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.livrable.new.title }),
  ).toBeTruthy();
});
