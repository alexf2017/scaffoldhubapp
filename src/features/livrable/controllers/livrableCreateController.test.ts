...
import { livrableCreateController } from 'src/features/livrable/controllers/livrableCreateController';
...
async function buildLivrable(context: AppContext) {
...
describe('livrableCreate', () => {
  ...
  it('must be signed in', async () => {
    const body = await buildLivrable({
      currentTenant,
      currentUser,
    ...
    try {
      await livrableCreateController(body, await testContext());
      fail();
    } catch (error) {
  ...
  it('must have permission', async () => {
    ...
    try {
      await livrableCreateController(
        body,
        await testContext({
