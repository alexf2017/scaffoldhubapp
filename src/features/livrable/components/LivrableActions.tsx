...
import { livrableDestroyManyApiCall } from 'src/features/livrable/livrableApiCalls';
...
export function LivrableActions({
  mode,
  livrable,
  context,
}: {
  ...
  const hasPermissionToEdit = hasPermission(permissions.livrableUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return livrableDestroyManyApiCall([livrable.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.livrable.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/livrable/${livrable.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/livrable/${livrable.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.livrable.destroy.confirmTitle}
