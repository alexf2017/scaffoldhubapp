...
import { statusDestroyManyApiCall } from 'src/features/status/statusApiCalls';
...
export function StatusActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.statusUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return statusDestroyManyApiCall([status.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.status.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/status/${status.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/status/${status.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.status.destroy.confirmTitle}
