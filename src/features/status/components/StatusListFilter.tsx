...
import { statusFilterFormSchema } from 'src/features/status/statusSchemas';
...
function StatusListFilter({
  ...
  const previewRenders = {
    name: {
      label: dictionary.status.fields.name,
    },
  };

  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof statusFilterFormSchema>>(
        searchParams,
        statusFilterFormSchema,
      ),
    [searchParams],
  );

  const form = useForm({
    resolver: zodResolver(statusFilterFormSchema),
    mode: 'onSubmit',
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.status.fields.name}</FormLabel>
