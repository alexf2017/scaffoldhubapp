...
import {
  projetDestroyManyApiCall,
  projetFindManyApiCall,
} from 'src/features/projet/projetApiCalls';
import { projetExporterMapper } from 'src/features/projet/projetExporterMapper';
import { projetFilterInputSchema } from 'src/features/projet/projetSchemas';
...
export default function ProjetListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof projetFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.projetCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.projetImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        projetExporterMapper(data.projets, context),
        dictionary.projet.fields,
        'projets',
      );
      toast({
        description: dictionary.projet.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.projet.destroyMany.noSelection);
      }

      return projetDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.projet.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/projet/importer`} prefetch={false}>
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.projet.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.projet.destroyMany.confirmDescription,
            selectedCount,
          )}
