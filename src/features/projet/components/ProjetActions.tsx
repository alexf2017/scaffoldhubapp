...
import { projetDestroyManyApiCall } from 'src/features/projet/projetApiCalls';
...
export function ProjetActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.projetUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return projetDestroyManyApiCall([projet.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.projet.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/projet/${projet.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/projet/${projet.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.projet.destroy.confirmTitle}
