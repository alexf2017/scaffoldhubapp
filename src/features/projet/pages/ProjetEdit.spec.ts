...
async function goToProjetForm(page: Page) {
  ...
  // TODO: Create projet here
  // await prisma.projet.create({});

  await page.waitForURL('/');
  await page.goto(`/projet/${projet.id}/edit`);
  await page.waitForURL(`/projet/${projet.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToProjetForm(page);
});
