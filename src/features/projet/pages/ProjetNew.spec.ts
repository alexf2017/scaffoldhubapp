...
async function goToProjetNew(page: Page) {
  ...
  await page.goto(`/projet/new`);
  await page.waitForURL(`/projet/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToProjetNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.projet.new.title }),
  ).toBeTruthy();
});
