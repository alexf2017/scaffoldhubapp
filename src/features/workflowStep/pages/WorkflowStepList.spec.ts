...
async function goToWorkflowStepList(page: Page) {
  ...
  // TODO: Create workflowStep here
  // await prisma.workflowStep.create({});

  await page.waitForURL('/');
  await page.goto(`/workflow-step`);
  await page.waitForURL(`/workflow-step`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToWorkflowStepList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.workflowStep.list.title }),
  ).toBeTruthy();
});
