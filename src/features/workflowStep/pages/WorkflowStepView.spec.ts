...
async function goToWorkflowStepView(page: Page) {
  ...
  await page.goto(`/workflow-step/${workflowStep.id}`);
  await page.waitForURL(`/workflow-step/${workflowStep.id}`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToWorkflowStepView(page);
});
