...
async function goToWorkflowStepNew(page: Page) {
  ...
  await page.goto(`/workflow-step/new`);
  await page.waitForURL(`/workflow-step/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToWorkflowStepNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.workflowStep.new.title }),
  ).toBeTruthy();
});
