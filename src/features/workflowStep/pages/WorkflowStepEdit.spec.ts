...
async function goToWorkflowStepForm(page: Page) {
  ...
  await page.goto(`/workflow-step/${workflowStep.id}/edit`);
  await page.waitForURL(`/workflow-step/${workflowStep.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToWorkflowStepForm(page);
});
