...
async function createWorkflowStep(context: AppContext) {
...
describe('workflowStepAutocomplete', () => {
  ...
  describe('validation', () => {
    it('must be signed in', async () => {
      try {
        await workflowStepAutocompleteController({}, await testContext());
        fail();
      } catch (error) {
  ...
  it.skip('filters', async () => {
    ...
    const data = await workflowStepAutocompleteController(
      { search },
      await testContext({
        currentUserId: currentUser?.id,
        currentTenantId: currentTenant?.id,
