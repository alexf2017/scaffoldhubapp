...
import { workflowStepUpdateController } from 'src/features/workflowStep/controllers/workflowStepUpdateController';
...
describe('workflowStepUpdate', () => {
  ...
  it.skip('must be signed in', async () => {
    const workflowStep = await createWorkflowStep(
      await testContext({
    ...
    try {
      await workflowStepUpdateController(
        { id: workflowStep.id },
        {},
        await testContext(),
      );
  ...
  //     const previousWorkflowStep = await createWorkflowStep(
  //       await testContext({
  //         currentUserId: currentUser?.id,
  //         currentTenantId: currentTenant?.id,
  //       }),
  ...
  //     const updatedWorkflowStep = await prisma.workflowStep.findUniqueOrThrow({
  //       where: { id: previousWorkflowStep?.id },
  //       include: {},
  //     });
