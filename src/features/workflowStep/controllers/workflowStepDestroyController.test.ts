...
import { workflowStepDestroyManyController } from 'src/features/workflowStep/controllers/workflowStepDestroyManyController';
...
describe('workflowStepDestroy', () => {
  ...
  it('must be signed in', async () => {
    try {
      await workflowStepDestroyManyController(
        {
          ids: [workflowStep?.id],
        },
        await testContext(),
  ...
  it.skip('deletes', async () => {
    const countBeforeDelete = await prisma.workflowStep.count();
    ...
    const countAfterDelete = await prisma.workflowStep.count();
