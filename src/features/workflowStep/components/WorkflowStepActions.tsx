...
import { workflowStepDestroyManyApiCall } from 'src/features/workflowStep/workflowStepApiCalls';
...
export function WorkflowStepActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.workflowStepUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return workflowStepDestroyManyApiCall([workflowStep.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.workflowStep.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/workflow-step/${workflowStep.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/workflow-step/${workflowStep.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.workflowStep.destroy.confirmTitle}
