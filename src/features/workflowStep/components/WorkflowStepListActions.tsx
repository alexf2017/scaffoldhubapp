...
import {
  workflowStepDestroyManyApiCall,
  workflowStepFindManyApiCall,
} from 'src/features/workflowStep/workflowStepApiCalls';
import { workflowStepExporterMapper } from 'src/features/workflowStep/workflowStepExporterMapper';
import { workflowStepFilterInputSchema } from 'src/features/workflowStep/workflowStepSchemas';
...
export default function WorkflowStepListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof workflowStepFilterInputSchema>;
  sorting: DataTableSort;
  count?: number;
  context: AppContext;
  table: Table<WorkflowStep>;
}) {
  ...
  const hasPermissionToCreate = hasPermission(permissions.workflowStepCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.workflowStepImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        workflowStepExporterMapper(data.workflowSteps, context),
        dictionary.workflowStep.fields,
        'workflowSteps',
      );
      toast({
        description: dictionary.workflowStep.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.workflowStep.destroyMany.noSelection);
      }

      return workflowStepDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.workflowStep.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/workflow-step/importer`} prefetch={false}>
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.workflowStep.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.workflowStep.destroyMany.confirmDescription,
            selectedCount,
          )}
