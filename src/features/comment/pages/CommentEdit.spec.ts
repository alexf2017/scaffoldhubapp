...
async function goToCommentForm(page: Page) {
  ...
  await page.goto(`/comment/${comment.id}/edit`);
  await page.waitForURL(`/comment/${comment.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToCommentForm(page);
});
