...
async function goToCommentNew(page: Page) {
  ...
  await page.goto(`/comment/new`);
  await page.waitForURL(`/comment/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToCommentNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.comment.new.title }),
  ).toBeTruthy();
});
