...
import { campanyDestroyManyApiCall } from 'src/features/campany/campanyApiCalls';
...
export function CampanyActions({
  mode,
  campany,
  context,
}: {
  ...
  const hasPermissionToEdit = hasPermission(permissions.campanyUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return campanyDestroyManyApiCall([campany.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.campany.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/campany/${campany.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/campany/${campany.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.campany.destroy.confirmTitle}
