...
async function goToCampanyNew(page: Page) {
  ...
  await page.goto(`/campany/new`);
  await page.waitForURL(`/campany/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToCampanyNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.campany.new.title }),
  ).toBeTruthy();
});
