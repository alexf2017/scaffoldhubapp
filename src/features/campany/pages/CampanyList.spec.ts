...
async function goToCampanyList(page: Page) {
  ...
  await page.goto(`/campany`);
  await page.waitForURL(`/campany`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToCampanyList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.campany.list.title }),
  ).toBeTruthy();
});
