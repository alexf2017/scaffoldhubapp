...
async function goToCampanyForm(page: Page) {
  ...
  await page.goto(`/campany/${campany.id}/edit`);
  await page.waitForURL(`/campany/${campany.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToCampanyForm(page);
});
