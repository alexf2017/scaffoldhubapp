...
import { workflowDestroyManyApiCall } from 'src/features/workflow/workflowApiCalls';
...
export function WorkflowActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.workflowUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return workflowDestroyManyApiCall([workflow.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.workflow.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/workflow/${workflow.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/workflow/${workflow.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.workflow.destroy.confirmTitle}
