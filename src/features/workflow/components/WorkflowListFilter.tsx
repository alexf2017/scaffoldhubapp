...
import { workflowFilterFormSchema } from 'src/features/workflow/workflowSchemas';
...
function WorkflowListFilter({
  ...
  const previewRenders = {
    title: {
      label: dictionary.workflow.fields.title,
    },
    steps: {
      label: dictionary.workflow.fields.steps,
      render: dataTableFilterRenders(context).relationToMany(stepsLabel),
    },
  };

  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof workflowFilterFormSchema>>(
        searchParams,
        workflowFilterFormSchema,
      ),
    [searchParams],
  );

  const form = useForm({
    resolver: zodResolver(workflowFilterFormSchema),
    mode: 'onSubmit',
  ...
  useEffect(() => {
    form.reset({ ...emptyValues, ...filter } as z.infer<
      typeof workflowFilterFormSchema
    >);
  }, [filter, form]);
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.workflow.fields.title}</FormLabel>
