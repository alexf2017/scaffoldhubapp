...
async function goToWorkflowForm(page: Page) {
  ...
  await page.goto(`/workflow/${workflow.id}/edit`);
  await page.waitForURL(`/workflow/${workflow.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToWorkflowForm(page);
});
